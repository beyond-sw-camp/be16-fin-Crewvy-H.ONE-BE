...
public class ReservationTypeController {
    ...
    public ResponseEntity<?> getAvailableResourceList(@RequestParam UUID companyId) {
    ...
    public ResponseEntity<?> getResourceStatusList() {
        List<ReservationEnumRes> statusList = Arrays.stream(ReservationTypeStatus.values())
                .map(status -> new ReservationEnumRes(status.name(), status.getCodeValue(), status.getCodeName()))
                .collect(Collectors.toList());
        return new ResponseEntity<>(new ApiResponse<>(true, statusList, "자원 상태 조회 성공"), HttpStatus.OK);
    }
}
