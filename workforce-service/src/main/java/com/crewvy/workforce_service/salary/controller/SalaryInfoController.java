...
public class SalaryInfoController {
    ...
    // 급여 기본 정보 전체 목록 조회
    @GetMapping("/list")
    public ApiResponse<List<SalaryInfoRes>> getSalaryInfos(@RequestParam UUID companyId) {
        List<SalaryInfoRes> response = salaryInfoService.getSalaryInfoList(companyId);
        return new ApiResponse<>(true, response, "전체 목록 조회 성공");
    }

    // 급여 기본 정보 저장
    @PostMapping("/save")
    public ApiResponse<List<SalaryInfoRes>> saveSalaryInfo(@RequestBody List<SalaryInfoCreateReq> requests) {
        List<SalaryInfoRes> response = salaryInfoService.saveSalaryInfoList(requests);
        return new ApiResponse<>(true, response, "항목 저장 성공");
    }

    // 급여 기본 정보 수정
    @PutMapping("/update")
    public ApiResponse<List<SalaryInfoRes>> updateSalaryInfo(@RequestBody List<SalaryInfoUpdateReq> requests) {
    ...
    public ApiResponse<?> deleteSalaryInfo(@RequestBody List<UUID> uuidList) {
